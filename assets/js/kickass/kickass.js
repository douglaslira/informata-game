 ...
 */
    (function () {
        ...
        var separator = /[, ]+/, elements = {
            ...
            hs: 1,
            rg: 1
        }, p2s = /,?([achlmqrstvxz]),?/gi, pathCommand = /([achlmqstvz])[\s,]*((-?\d*\.?\d*(?:e[-+]?\d+)?\s*,?\s*)+)/ig, pathValues = /(-?\d*\.?\d*(?:e[-+]?\d+)?)\s*,?\s*/ig, radial_gradient = /^r(?:\(([^,]+?)\s*,\s*([^\)]+?)\))?/, sortByKey = function (a, b) {
            return a.key - b.key;
        };
        ...
        var pathDimensions = cacher(function (path) {
            ...
            return {x: xmin, y: ymin, width: mmax[apply](0, X) - xmin, height: mmax[apply](0, Y) - ymin};
            ...
            var p = pathToAbsolute(path), p2 = path2 && pathToAbsolute(path2), attrs = {
                ...
                qx: null,
                qy: null
            }, attrs2 = {x: 0, y: 0, bx: 0, by: 0, X: 0, Y: 0, qx: null, qy: null}, processPath = function (path, d) {
                var nx, ny;
                if (!path) {
        ...
        if (R.svg) {
            ...
            var theCircle = function (svg, x, y, r) {
                ...
                width = width || 512;
                height = height || 342;
                $(cnvs, {xmlns: "http://www.w3.org/2000/svg", version: 1.1, width: width, height: height});
                if (container == 1) {
        ...
        if (R.vml) {
            ...
            thePath = function (pathString, vml) {
                ...
                g[appendChild](el);
                var p = new Element(el, g, vml), attr = {fill: "none", stroke: "#000"};
            ...
            if (x != 0) {
                ...
                switch (this.type) {
                    ...
                    case"image":
                        ...
                        this.attr({height: newh, r: a.r * fr, width: neww, x: ncx - neww / 2, y: ncy - newh / 2});
                        break;
                    case"circle":
                    case"ellipse":
                        this.attr({rx: a.rx * kx, ry: a.ry * ky, r: a.r * fr, cx: ncx, cy: ncy});
                        break;
                    case"text":
        ...
        Set[proto].getBBox = function () {
            ...
            x = mmin[apply](0, x);
            y = mmin[apply](0, y);
            return {x: x, y: y, width: mmax[apply](0, w) - x, height: mmax[apply](0, h) - y};
        };
    ...
    (function (window, document, location, setTimeout, decodeURIComponent, encodeURIComponent) {
        ...
        easyXDM.DomHelper = {
            on: on, un: un, requiresJSON: function (path) {
            ...
            return (pub = {
                outgoing: function (message, domain, fn) {
                    ...
                    callerWindow = createFrame({
                        props: {src: config.local + "#_4" + config.channel}, onLoad: function onLoad() {
    ...
    var GameGlobals = {
        FPS: 60, useAnimationFrame: false, boids: {flockRadius: 400, size: 100}, path: function () {
            return document.location.protocol + "//localhost/snake-game/" + Array.prototype.slice.call(arguments).join("");
    ...
    var KickAss = new Class({
        initialize: function (options) {
            ...
            this.ui.showMessage("WOW!!!. Você subiu de nível: <strong>" + rank + '</strong>!');
        }, fireBomb: function () {
            ...
            return window.KICKASSVERSION == "GAME_UK" || window.IS_CLOUDFLARE_GAME;
        }, isMySite: function () {
            return !!window.KICKASS_SITE_KEY;
    ...
    var Menu = new Class({
        initialize: function (game) {
            ...
            var shipId = window.KICKASSSHIPID || "";
            this.url = GameGlobals.path('intermediate.html?url=' +
                encodeURIComponent(window.KICKASSURL || document.location.href) + "&preship=" + (shipId) + "&is_campaign=" + (this.game.isCampaign() ? "true" : "") + "&is_mysite=" + (this.game.isMySite() ? "true" : ""));
            ...
            this.socket = new easyXDM.Socket({
                ...
                props: {
                    ...
                    style: {width: '100%', height: this.size.height + 'px'}
                }
            });
    ...
    var MenuManager = new Class({
        initialize: function () {
            ...
            // if (typeof window.KICKASS_SHARE_URL !== "undefined") {
            //     if (window.KICKASS_SHARE_URL) {
            //         var url = encodeURIComponent(window.KICKASS_SHARE_URL);
    ...
    var Thruster = new Class({
        initialize: function (data, ship) {
            ...
            var redWidth = this.size.width, redIncrease = this.size.width * 0.05, yellowWidth = this.size.width * 0.8, yellowIncrease = yellowWidth * 0.1, halfRed = redWidth / 2, halfYellow = yellowWidth / 2, offsetY = -this.size.height / 2, metaY = 0;
            var px = this.pos.x;
    ...
    var BombManager = new Class({
        initialize: function () {
            ...
            if (this.nextBomb < 0) {
                ...
                this.nextBomb = -1;
                this.game.ui.showMessage("BOMBA ESTÁ PRONTA <br />(TECLE F)");
            }
        }, blow: function () {
    ...
    var LaserCannon = new Class({
        Extends: Cannon, initialize: function (x, y, angle) {
    ...
    var SplitExplosion = new Class({
        Extends: Explosion, initialize: function (pos, element) {
            ...
            setStyles(left, {'WebkitTransform': lT, 'MozTransform': lT, 'OTransform': lT, 'MsTransform': lT});
            setStyles(right, {'WebkitTransform': rT, 'MozTransform': rT, 'OTransform': rT, 'MsTransform': rT});
            this.left = left;
            this.right = right;
    ...
    var Sheet = new Class({
        initialize: function (rect) {
            ...
            if (!GameGlobals.hasCanvas)
                this.drawer = new SheetRaphael(rect); else
    ...
    var initKickAss = function () {
        ...
        if (!window.KICKASSGAME) {
            if (window.KICKASS_SITE_KEY) {
                var mySite = new MySite(window.KICKASS_SITE_KEY);
